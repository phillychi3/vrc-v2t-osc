    ...
    def test_stream_matches_whole_signal_without_frame_drift(self):
        for rate in (8000, 16000, 22050, 44100, 48000, 96000):
            with self.subTest(rate=rate):
                ...
                expected = (
                    AudioSegment(pcm, sample_width=2, frame_rate=rate, channels=1)
                ...
                with patch(
                    "subprocess.Popen", side_effect=AssertionError("FFmpeg invoked")
                ):
                    for start in range(0, len(pcm), 1024 * 2):
                        block, state = normalize_audio(
                            pcm[start : start + 2048], 1, rate, state
                        )
                        output.append(block)
        ...
        for channels in (2, 6):
            with self.subTest(channels=channels):
                ...
                self.assertLessEqual(
                    abs(struct.unpack("<h", converted)[0]), channels // 2
                )
